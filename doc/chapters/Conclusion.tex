...
The cavity model (Eqs.~\ref{eq:fres}--\ref{eq:ae}) gives a physical radius of $19.87$~mm on the final FR-408HR stack ($\epsilon_r=3.68$, $h=0.1$~mm prepreg), within $0.5$~mm of the HFSS-tuned radius used in \texttt{CircularPatchV3.aedt}. The early rectangular $\lambda_g/2$ estimate ($L\approx 17.8$~mm on RO4350B) and the hexagonal variant were evaluated and superseded. The MATLAB particle-swarm search (\path{matlab/MPA_opt_swrm.m}) demonstrated the limits of a single-layer lossless model, converging to a low-impedance minimum ($Z\approx 0.3$~$\Omega$, RL${}\approx 0$~dB) that confirmed the need for direct HFSS tuning of patch radius and feed offset.
